...
ax_vsd_recall = MK.Axis(fig_recall[2, 1]; xlabel="error threshold", ylabel="recall", yticks=[0, 0.5, 1], limits=(nothing, (0, 1)), title="VSD")
ax_adds_recall = MK.Axis(fig_recall[2, 2]; xlabel="error threshold", ylabel="recall", yticks=[0, 0.5, 1], limits=(nothing, (0, 1)), title="ADDS")
gl_recall = fig_recall[1, :] = MK.GridLayout()
ax_vsdbop_recall = MK.Axis(gl_recall[1, 1]; xlabel="error threshold", ylabel="recall", yticks=[0, 0.5, 1], limits=(nothing, (0, 1)), title="VSDBOP")
...
MK.vspan!(ax_vsdbop_density, first(BOP19_THRESHOLDS), last(BOP19_THRESHOLDS))
MK.vlines!(ax_vsd_density, BOP18_θ)
MK.vlines!(ax_adds_density, ADDS_θ)
MK.Legend(gl_density[1, 2], ax_vsdbop_density)
# display(fig_density)
save(joinpath("plots", "$(experiment_name)_density.pdf"), fig_recall)
