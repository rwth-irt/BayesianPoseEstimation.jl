...
cb = MCMCDepth.heatmap_colorbar!(fig, ax_exp, hm; label="", ticks=([minimum(render_img[render_img.>0]) + 0.01, maximum(render_img) - 0.01], ["close", "far"]))
display(fig)
...
cb = MCMCDepth.heatmap_colorbar!(fig, ax_mask, hm; label="", ticks=([minimum(masked_img[masked_img.>0]) + 0.01, maximum(masked_img) - 0.01], ["close", "far"]))
display(fig)
