...
function run_inference(parameters::Parameters, render_context, obs; kwargs...)
    ...
    chain = sample(rng, posterior, ind_sym_sampler, 10_000; discard_initial=0_000, thinning=1, kwargs...)
...
plt_o_chain = plot_variable(model_chain, :o, STEP; label=["x" "y" "z"], xlabel="Iteration [÷ $(STEP)]", ylabel="Zugehörigkeit", legend=false)
plt_o_dens = density_variable(model_chain, :o; label=["x" "y" "z"], xlabel="Zugehörigkeit [0,1]", ylabel="Wahrscheinlichkeit", legend=false)
