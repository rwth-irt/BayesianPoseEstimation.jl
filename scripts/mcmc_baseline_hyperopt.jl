...
function parse_hyperopt_config(path)
...
function scene_inference(gl_context, config)
    ...
    # Load best parameters for dataset
    pro_df = collect_results(datadir("exp_raw", "mcmc_mh_hyperopt"))
    transform!(pro_df, :path => ByRow(parse_hyperopt_config) => [:dataset, :model])
    ...
    @reset parameters.proposal_σ_t = fill(best_params[:proposal_σ_t], 3)
